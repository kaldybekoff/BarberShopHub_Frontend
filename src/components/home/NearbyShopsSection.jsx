      ...
      <div className="flex gap-3 overflow-x-auto pb-2">
        {shops.map((shop) => (
          ...
          >
            ...
            <div
              className="w-full h-24 rounded-xl flex items-center justify-center text-3xl mb-1"
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-1">
                ...
                <span className="text-xs text-white font-medium">{shop.rating}</span>
                <span className="text-xs" style={{ color: colors.gray }}>({shop.reviewCount})</span>
              </div>
              <span className="text-xs" style={{ color: colors.gray }}>от {shop.priceFrom} ₸</span>
            </div>

            <button
              onClick={() => navigate(`/shops/${shop.id}`)}
              className="w-full py-2 rounded-xl text-xs font-semibold text-white mt-1"
